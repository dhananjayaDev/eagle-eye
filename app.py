...
# Global state variables for stop light violation detection
vehicle_positions = {}  # {video_path: {track_id: deque([(x, y, frame_count), ...])}} for tracking movement
traffic_light_states = {}  # {video_path: state} to track traffic light state per video
prev_gray_frames = {}  # {video_path: prev_gray} to store previous grayscale frame per video
...
# Define vehicle and traffic light classes
...
def is_dashcam_stopped(frame, video_path, threshold=0.4):
    """Checks if the dashcam vehicle is stationary using optical flow."""
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    if video_path not in prev_gray_frames or prev_gray_frames[video_path] is None:
        prev_gray_frames[video_path] = gray
        return False

    flow = cv2.calcOpticalFlowFarneback(prev_gray_frames[video_path], gray, None, 0.5, 3, 15, 3, 5, 1.2, 0)
...
def get_traffic_light_color(roi):
    ...
    min_area_threshold = 100  # Minimum pixel area to consider a color valid
    red_area = np.sum(mask_red)
    green_area = np.sum(mask_green)

    if red_area > green_area and red_area > min_area_threshold:
        return "Red"
    elif green_area > red_area and green_area > min_area_threshold:
        return "Green"
    return "Unknown"
...
def video_feed(filename):
    file_path = os.path.join(app.config["UPLOAD_FOLDER"], filename)
    logger.info(f"Starting video feed for: {file_path}")
    return Response(process_video(file_path, event_type="critical"),
                    mimetype="multipart/x-mixed-replace; boundary=frame")
...
def process_video(video_path, event_type="critical"):
    ...
    # Initialize state variables for this video
    vehicle_positions[video_path] = {}
    traffic_light_states[video_path] = "--"
    prev_gray_frames[video_path] = None
    ...
    try:
        with app.app_context():
            while cap.isOpened():
                ...
                frame_count += 1
                logger.debug(f"Processing frame {frame_count}")
                # Removed frame skipping to ensure smoother streaming
                # if frame_count % 2 == 0:
                #     continue
                frame = cv2.resize(frame, (640, 480))
                height, width, _ = frame.shape
                ...
                # Check if dashcam is stopped for environment mode
                dashcam_stopped = False
                if event_type == "environment":
                    dashcam_stopped = is_dashcam_stopped(frame, video_path)

                    # Display dashcam status
                    if dashcam_stopped:
                        cv2.putText(frame, "Dashcam Stabilized", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                ...
                # Prepare detections with class IDs for SORT tracker
                for box in boxes:
                    ...
                    if event_type == "pedestrian":
                        # Only include pedestrians (class ID 0) in pedestrian mode
                        if class_id == 0:
                            # Include class_id in the detection array: [x1, y1, x2, y2, class_id]
                            detections.append([x1, y1, x2, y2, class_id])
                    elif event_type == "environment":
                        # Include vehicles and traffic lights for environment mode
                        if class_name in VEHICLE_CLASSES or class_name == TRAFFIC_LIGHT_CLASS:
                            detections.append([x1, y1, x2, y2, class_id])
                    else:
                        # For critical mode, include vehicles and pedestrians
                        if class_id in [0, 2, 3, 5, 7]:  # 0: person, 2: car, 3: motorcycle, 5: bus, 7: truck
                            detections.append([x1, y1, x2, y2, class_id])

                # Update tracker with detections (including class IDs)
                if detections:
                    # SORT expects [x1, y1, x2, y2, score] format, but we'll append class_id after tracking
                    tracked_objects = tracker.update(np.array([d[:4] for d in detections]))
                    # Map class IDs to tracked objects based on closest match of bounding boxes
                    tracked_with_class = []
                    for track in tracked_objects:
                        x1, y1, x2, y2, track_id = map(int, track)
                        # Find the closest detection to this track to assign the class ID
                        best_iou = 0
                        assigned_class_id = None
                        for det in detections:
                            det_x1, det_y1, det_x2, det_y2, det_class_id = det
                            iou = calculate_iou([x1, y1, x2, y2], [det_x1, det_y1, det_x2, det_y2])
                            if iou > best_iou:
                                best_iou = iou
                                assigned_class_id = det_class_id
                        # Append the class ID to the track: [x1, y1, x2, y2, track_id, class_id]
                        if assigned_class_id is not None:
                            tracked_with_class.append([x1, y1, x2, y2, track_id, assigned_class_id])
                        else:
                            # If no match found, skip this track
                            continue
                else:
                    tracked_objects = tracker.update(np.empty((0, 4)))
                    tracked_with_class = []

                test_data = []
                for track in tracked_with_class:
                ...
                if test_data and event_type == "critical":
                    ...
                    frontier_idx = np.argmax(predictions) if any(predictions) else -1
                    logger.info(f"Frontier vehicle index: {frontier_idx}, Vehicle: {frontier_vehicle}")
                    if frontier_idx >= 0 and frontier_idx < len(tracked_with_class):
                        frontier_vehicle = tracked_with_class[frontier_idx]
                        if frontier_vehicle is not None:
                            track_id = int(frontier_vehicle[4])
                            y_center = (frontier_vehicle[1] + frontier_vehicle[3]) // 2
                            frontier_speed = estimate_frontier_speed(track_id, y_center, frame_count, FRAME_TIME)
                ...
                for track in tracked_with_class:
                    ...
                    # Determine class name directly from the tracked class_id
                    class_name = COCO_CLASSES[class_id] if class_id < len(COCO_CLASSES) else ""
                    ...
                    if event_type == "critical" and np.array_equal(track, frontier_vehicle):
                        ...
                        if ttc is not None and ttc < 0.3:
                            is_collision = True
                        for other_track in tracked_with_class:
                            if not np.array_equal(other_track, track) and len(other_track) >= 6:
                                ox1, oy1, ox2, oy2, other_id, _ = other_track
                                iou = calculate_iou([x1, y1, x2, y2], [ox1, oy1, ox2, oy2])
                                if iou > 0.5:
                                    is_collision = True
                                    collided_vehicles.add(other_id)
                        ...
                        if class_name == TRAFFIC_LIGHT_CLASS:
                            roi = frame[y1:y2, x1:x2]
                            detected_color = get_traffic_light_color(roi)
                            if detected_color in ["Red", "Green"]:
                                traffic_light_states[video_path] = detected_color

                            cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 0), 1)
                            cv2.putText(frame, f"Light: {traffic_light_states[video_path]}", (x1, y1 - 10),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 155), 1)
                            continue

                        # Handle vehicle detection and stop light violation
                        if class_name in VEHICLE_CLASSES:
                            center_x = (x1 + x2) // 2
                            center_y = (y1 + y2) // 2

                            # Track vehicle position over frames to determine movement
                            if track_id not in vehicle_positions[video_path]:
                                vehicle_positions[video_path][track_id] = []
                            vehicle_positions[video_path][track_id].append((center_x, center_y, frame_count))

                            is_moving = False
                            if len(vehicle_positions[video_path][track_id]) >= 2:
                                prev_x, prev_y, prev_frame = vehicle_positions[video_path][track_id][-2]
                                curr_x, curr_y, curr_frame = vehicle_positions[video_path][track_id][-1]
                                dist_moved = np.sqrt((curr_x - prev_x) ** 2 + (curr_y - prev_y) ** 2)
                                time_diff = (curr_frame - prev_frame) * FRAME_TIME
                                speed = dist_moved / time_diff if time_diff > 0 else 0
                                is_moving = speed > 5  # Threshold for movement (pixels per second)

                            color = (0, 255, 0)  # Green if not moving
                            if traffic_light_states[video_path] == "Red" and dashcam_stopped and is_moving:
                                color = (0, 0, 255)  # Red if moving on a red light
                                event_type_local = "Red Light Violation"
                                vehicle_motion = "Moving on Red Light"
                            else:
                                event_type_local = "Environment"
                                vehicle_motion = "Traffic Object Detected"

                            # Keep only the last 10 positions to save memory
                            if len(vehicle_positions[video_path][track_id]) > 10:
                                vehicle_positions[video_path][track_id].pop(0)
                    ...
                    is_critical = vehicle_motion in ["Collided", "Harsh Braking", "Sudden Stop Detected!", "Moving on Red Light"]
                    bg_color = (0, 0, 255) if is_critical else (0, 0, 0)
                    alpha = 0.6
                ...
                if event_type == "environment":
                    cv2.putText(frame, f"Traffic Light: {traffic_light_states[video_path]}", (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 155), 1)
        ...
        cap.release()
        # Clean up state variables for this video
        vehicle_positions.pop(video_path, None)
        traffic_light_states.pop(video_path, None)
        prev_gray_frames.pop(video_path, None)
        with app.app_context():
            db.session.remove()
...
def export_critical_events():
    try:
        with app.app_context():
            critical_event_types = ["Collided", "Harsh Braking", "Sudden Stop Detected!", "Moving on Red Light"]
            critical_events = EventLog.query.filter(EventLog.motion_status.in_(critical_event_types)).all()
